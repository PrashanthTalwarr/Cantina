...
| Metric | Target | What It Measures | Where Tracked |
|--------|--------|-----------------|---------------|
...
After every discovery call, the caller fills out (2 minutes). Responses feed directly into the `discovery_calls` table in PostgreSQL, which drives scoring model recalibration.
...
- **Action:** Double down. Automate full pipeline on AWS Lambda (daily cron). Scale from 8 targets to 50+/week. Activate event-triggered outreach via `src/monitoring/event_monitor.py` — exploit or funding round detected → contextual email within 24 hours.

**YELLOW — Refine (4–5 confirm)**
- 4–5 confirm but severity is moderate (5–6/10)
- "It's on our roadmap but not urgent"
- **Action:** Tighten ICP. Check if pain is sharper for specific categories (bridges > DEXs?) or protocol stages (pre-launch > established). A/B test messaging angles — fear vs. bottleneck framing.

**RED — Pivot (fewer than 4 confirm)**
- Common objection: "We handle this internally" or "Our current auditors are fine"
- **Action:** Hypothesis A as framed doesn't resonate. Investigate: wrong messaging, wrong audience, or wrong hypothesis? Run 5 calls positioned around tool consolidation (Hypothesis B) to compare. Check if the `reachability` score needs more weight — unreachable teams inflate call counts without quality.

---

## Scoring Model Recalibration

After 10 calls, discovery data recalibrates scoring weights in `config/scoring_weights.json`. The recalibration script:

1. Queries `discovery_calls` table for confirmed pain + next-step conversion data
2. Correlates with `lead_scores` factor breakdown
3. Computes which factors best predicted positive outcomes
4. Updates `config/scoring_weights.json` — no code changes, just config
5. Logs the version change in `scoring_model_versions` table
6. Sends a Slack alert with the before/after weight diff

Example: if `audit_status` predicted conversion 3× better than `tvl`:
...
The pipeline picks up the new weights automatically on the next run.
...
- "We use Immunefi but the quality of submissions is low" → Cantina's triage and researcher quality is the differentiator
